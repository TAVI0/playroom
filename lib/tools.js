...
// Tool determinística: en vez de sumar un campo mas a los chunks del RAG
// (mas competencia en el top_k del retrieval por algo que se pregunta seguro,
// no "tal vez"), se lee directo de la fuente de verdad -- mismo criterio que
// listar_experiencia_completa y listar_skills_tecnicas.
export const busquedaLaboral = tool(
	async () => {
		console.log("TOOL llamada: busqueda_laboral (sin argumentos)");
		const resultado = infoVerificada.busquedaLaboral;
		console.log("TOOL resultado:", resultado);
		return JSON.stringify(resultado);
	},
	{
		name: "busqueda_laboral",
		description:
			"Usar cuando pregunten que tipo de rol busca Marcos, en que modalidad (remoto/" +
			"hibrido/presencial), en que ubicacion, o si tiene disponibilidad para empezar. " +
			"Devuelve roles, modalidad, ubicacion y disponibilidad exactos.",
		schema: z.object({}),
	},
);

// Pool de rechazos con la misma idea (redirigir a otra IA/buscador) pero
// distinta forma -- una sola frase fija se sentía repetitiva en charlas
// largas. Se elige una al azar en el servidor (no se le pide al modelo que
// improvise, para no perder el tono) y la respuesta final debe reproducirla
// tal cual, igual que descargar_cv y datos_de_contacto.
const RECHAZOS_FUERA_DE_TEMA = [
	"Eso preguntale a ChatGPT, yo estoy acá para hablarte de Marcos Tavio 😄",
	"Seguro que en Google encontrás algo así, yo solo sé de Marcos Tavio 🔍",
	"Capaz que Claude te lo responde mejor, yo soy experto en un solo tema: Marcos Tavio 🤓",
	"Ni idea, no es lo mío -- preguntame algo sobre Marcos Tavio, ahí te tiro con todo 📎",
];

export const rechazarPreguntaFueraDeTema = tool(
	async () => {
		console.log("TOOL llamada: rechazar_pregunta_fuera_de_tema (sin argumentos)");
		const resultado =
			RECHAZOS_FUERA_DE_TEMA[Math.floor(Math.random() * RECHAZOS_FUERA_DE_TEMA.length)];
		console.log("TOOL resultado:", resultado);
		return resultado;
	},
	{
		name: "rechazar_pregunta_fuera_de_tema",
		description:
			"Usar SIEMPRE que la pregunta no tenga NADA que ver con Marcos Tavio (temas generales, " +
			"otras personas, pedidos de codigo, cultura general, etc.). No intentes responder la " +
			"pregunta original ni busques contexto con otra tool -- llamá directamente a esta.",
		schema: z.object({}),
	},
);
